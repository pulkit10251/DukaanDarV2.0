...
export const authenticate = (userId, token, expiryTime, type) => {
  return (dispatch) => {
...
export const signup = (email, password, name, contact, type) => {
  return async (dispatch) => {
    ...
    const allshopIdsresponse = await fetch(
      `https://dukaandar-e4590.firebaseio.com/AllShopIds.json?auth=${resData.idToken}`
    )

    const allshopids = await allshopIdsresponse.json();
    var newShopId;
    do{
      newShopId = "DD" + Math.floor(1000 + Math.random() * 9000);
    }while(allshopids[newShopId] === undefined);
